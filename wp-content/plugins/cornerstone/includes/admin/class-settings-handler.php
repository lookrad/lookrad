	...
	public function __construct() {
		...
		if ( isset( $_POST['cornerstone_options_submitted'] ) && isset($_POST['cornerstone-fields']) ) {
  		...
  		if ( strip_tags( $_POST['cornerstone_options_submitted'] ) == 'submitted' && current_user_can( 'manage_options' ) ) {
  		 	...
  		 	foreach ( $fields as $name => $value ) { //remove html tags
    ...
    $current_value = self::$settings[ $name ];
		...
		switch ( $options['type'] ) {
			...
			case 'checkbox':
				...
				if ( is_array( $options['value'] ) ) { // If it's multiple selection - return multiple checkbox
				...
				return sprintf( // Return single checkbox
					$checkbox,
